...
def _simulate_clamped_ef(input_ef: float, variability: int = 10, kurtosis: float = KURTOSIS, num_simulations: int = NUM_SIMULATIONS) -> np.ndarray:
    """Helper function to simulate clamped exposure factors using a Beta distribution.

    The clamped exposure factor is generated by sampling from a Beta distribution with parameters adjusted for the desired kurtosis. The sampled values are then clamped to be within ± variability of the input EF. This function simulates uncertainty in the provided EF (percentage of asset value at risk).

    Args:
        input_ef: The input exposure factor to clamp around.
        variability: The percentage variability around the input EF (default is 10).
    ...
    alpha, beta_param = get_beta_parameters_for_kurtosis(kurtosis)  # example
    raw = beta(a=alpha, b=beta_param).rvs(num_simulations)
    return lower_bound + raw * (upper_bound - lower_bound)


def _simulate_clamped_aro(input_aro: float, variability: int = 10, num_simulations: int = NUM_SIMULATIONS) -> np.ndarray:
    """Helper function to simulate clamped Annual Rate of Occurrence (ARO) values.

    The clamped ARO is generated by sampling from a Poisson distribution with a mean adjusted for the desired variability around the input ARO. The sampled values are then clamped to be within ± variability of the input ARO. This function simulates uncertainty in the provided ARO (frequency of the risk event).

    Args:
        input_aro: The input annual rate of occurrence to clamp around.
        variability: The percentage variability around the input ARO (default is 10).
        num_simulations: The number of simulations to generate (default is the constant NUM_SIMULATIONS).
...
def _simulate_losses_monte_carlo(
    ...
    simulated_EF = _simulate_clamped_ef(exposure_factor, kurtosis, num_simulations)
    simulated_ARO = _simulate_clamped_aro(annual_rate_of_occurrence, num_simulations)
...
def _simulate_losses_with_mcmc(
    ...
    # Use clamped EF and ARO
    aro_samples = _simulate_clamped_aro(annual_rate_of_occurrence, 10, num_simulations)
    simulated_ef = _simulate_clamped_ef(exposure_factor, 10, kurtosis, num_simulations)
    ...
    # Apply control effectiveness to scaled EF and ARO
...
def plot_risk_calculation_before_after(
    ...
    The simulation uses:
        - **Beta Distribution**: To simulate the Exposure Factor (EF), representing the percentage of asset value that is at risk during a risk event. The Beta distribution allows for variability in the EF, with the distribution's shape controlled by parameters that introduce kurtosis (peakedness). The implementation uses a default variability of 10% around the input EF to model uncertainty and possible outcomes around the input EF.
        - **Poisson Distribution**: To simulate the Annual Rate of Occurrence (ARO), representing the frequency of the risk event over a year. It models the randomness of how often the event will occur. After applying controls, the ARO is reduced by a specified percentage and recalculated using the Poisson distribution. The implementation uses a default variability of 10% around the input ARO to model uncertainty and possible outcomes around the input ARO.
